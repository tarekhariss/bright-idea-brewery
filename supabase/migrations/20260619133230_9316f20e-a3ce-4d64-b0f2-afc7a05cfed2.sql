...
BEGIN
  ...
  ELSIF v_link_deal_id IS NOT NULL THEN
    -- Atomic link of an existing deal (workspace-scoped check).
    IF EXISTS (SELECT 1 FROM public.deals WHERE id = v_link_deal_id AND workspace_id = v_workspace_id) THEN
