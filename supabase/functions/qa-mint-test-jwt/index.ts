...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "qa_seed_contacts") {
      ...
      const { data, error } = await admin.from("contacts").upsert(rows, { onConflict: "workspace_id,email_normalized", ignoreDuplicates: false }).select("id, email_normalized, email_canonical_status, email_status_source, email_status_verified_at, email_status_confidence, is_role_based, is_disposable, is_catch_all");
    ...
    if (action === "qa_inspect_contacts") {
      const emails: string[] = await req.json().then(b => b?.emails ?? []).catch(() => []);
      const { data, error } = await admin.from("contacts").select("email_normalized, email_canonical_status, email_status_source, email_status_verified_at, email_status_confidence, is_role_based, is_disposable, is_catch_all, is_free_email").eq("workspace_id", QA_WORKSPACE).in("email_normalized", emails);
