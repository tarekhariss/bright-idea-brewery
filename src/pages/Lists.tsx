...
export default function ListsPage() {
  ...
  const fetchLists = useCallback(async () => {
    ...
    // Fetch contact counts for static lists
  ...
  const handleCreate = async () => {
    ...
    const { error } = await db().from("lists").insert({
      ...
      is_dynamic: newType === "dynamic",
      filter_criteria: newType === "dynamic" ? {} : null,
      created_by: user.id,
    });
    if (!error) {
      setCreateOpen(false);
    ...
    <div className="p-6 space-y-4 animate-fade-in">
      ...
      {loading ? (
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-16 text-center">
            ...
            {lists.length === 0 && (
              <Button size="sm" className="mt-4 gap-1.5 text-xs" onClick={() => setCreateOpen(true)}>
                <Plus className="h-3.5 w-3.5" /> Create List
              </Button>
            )}
          </CardContent>
            ...
            >
              <CardContent className="p-4 space-y-2.5">
                <div className="flex items-start justify-between">
                  ...
                  <Badge
                    variant={list.is_dynamic ? "default" : "secondary"}
                ...
                <div className="flex items-center gap-3 text-[11px] text-muted-foreground pt-1">
                  <span className="flex items-center gap-1">
                    ...
                    {list.is_dynamic ? "Dynamic" : `${(counts[list.id] ?? 0).toLocaleString()} contacts`}
                  </span>
                  <span className="flex items-center gap-1">
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader><DialogTitle className="text-base">Create List</DialogTitle></DialogHeader>
            ...
            <div>
              ...
              <Select value={newType} onValueChange={(v) => setNewType(v as any)}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="dynamic">Dynamic — rule-based auto-matching</SelectItem>
                </SelectContent>
              </Select>
            </div>
          </div>
