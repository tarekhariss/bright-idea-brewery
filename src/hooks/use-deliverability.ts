...
export function useCreateDomain() {
  ...
  const { user } = useAuth();
  return useMutation({
    mutationFn: async (vals: Omit<SendingDomainInsert, "owner_id" | "created_by">) => {
      const { data, error } = await from("sending_domains")
      ...
      if (error) throw error;
      // Activity log
      await from("system_activity_log").insert({
        action: "domain_created",
        entity_type: "sending_domain",
        entity_id: data.id,
        performed_by: user?.id,
        details: { domain_name: vals.domain_name },
      });
      return data as SendingDomain;
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["sending_domains"] });
...
export function useUpdateDomain() {
  ...
  return useMutation({
    ...
    onSuccess: (d) => {
      qc.invalidateQueries({ queryKey: ["sending_domains"] });
      qc.invalidateQueries({ queryKey: ["sending_domains", d.id] });
    },
    onError: (e: any) => toast.error(e.message),
  });
}
...
export function useCreateMailbox() {
  ...
  const { user } = useAuth();
  return useMutation({
    mutationFn: async (vals: Omit<MailboxInsert, "owner_id" | "created_by">) => {
      const { data, error } = await from("mailboxes")
        .insert({ ...vals, owner_id: user?.id, created_by: user?.id })
        .select()
        .single();
      ...
      await from("system_activity_log").insert({
        ...
        entity_id: data.id,
        performed_by: user?.id,
        details: { email: vals.email, provider_type: vals.provider_type },
      });
...
export function useUpdateMailbox() {
  ...
  return useMutation({
    ...
    onSuccess: (d) => {
      qc.invalidateQueries({ queryKey: ["mailboxes"] });
      qc.invalidateQueries({ queryKey: ["mailboxes", d.id] });
    },
    onError: (e: any) => toast.error(e.message),
  });
}
