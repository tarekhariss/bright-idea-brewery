...
import {
  ...
  Plus, Trash2, RefreshCw, ExternalLink, Shield,
} from "lucide-react";
...
export function GoogleConnectionWizard({ open, onClose }: Props) {
  const { data: accounts, isLoading } = useProviderConnectionsByType("google");
  ...
  const handleConnect = async () => {
    ...
    // In production: initiate OAuth flow. For now: store placeholder.
    toast.info("OAuth flow would start here. Storing connection metadata...");
    await createConn.mutateAsync({
      workspace_id: "placeholder", // Will be set by RLS context
      ...
      <DialogContent className="sm:max-w-2xl max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <div className="flex items-center gap-3">
            ...
            <div>
              <DialogTitle className="text-base">Google Workspace</DialogTitle>
              <DialogDescription className="text-sm">Connect Gmail accounts via OAuth for email sending.</DialogDescription>
            </div>
          </div>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-lg bg-muted/50 p-3 flex items-start gap-2">
            ...
            <div className="text-xs text-muted-foreground">
              ...
              <p className="mt-0.5">When activated, this will use Google OAuth to securely connect your account without storing passwords. Currently in setup mode.</p>
            </div>
          </div>
              ...
              <TableBody>
                {accounts.map((a) => (
                  <TableRow key={a.id}>
                    ...
                    <TableCell>
                      <Badge variant="secondary" className="text-[10px] capitalize">{a.oauth_token_status || "none"}</Badge>
                    </TableCell>
          ...
          {addMode ? (
            <div className="space-y-3 rounded-lg border p-4">
              <p className="text-xs font-semibold">Connect Google Account</p>
